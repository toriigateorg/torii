...
<script>(function(){
  ...
  root.innerHTML = [
    ...
    '.btn{position:fixed;width:40px;height:40px;border-radius:9999px;border:1.5px solid #1a1a1a;padding:0;background:#ffffff;display:inline-flex;align-items:center;justify-content:center;cursor:grab;touch-action:none;user-select:none;-webkit-user-select:none;box-shadow:0 6px 16px -4px rgba(0,0,0,.25),0 2px 4px -1px rgba(0,0,0,.15);transition:transform .15s ease,box-shadow .15s ease;z-index:2147483647;}',
    '.btn:hover{transform:scale(1.06);box-shadow:0 8px 22px -4px rgba(0,0,0,.55),0 2px 4px -1px rgba(0,0,0,.3);}',
    ...
    '.btn .logo{width:22px;height:22px;display:block;}',
    ...
    '@media (prefers-color-scheme: light){.menu{background:rgba(255,255,255,.96);color:#111;border-color:rgba(0,0,0,.12);box-shadow:0 12px 40px -8px rgba(0,0,0,.18),0 4px 12px -2px rgba(0,0,0,.08);} .menu .label{color:rgba(0,0,0,.5);} .menu .item:hover,.menu .item:focus-visible{background:rgba(0,0,0,.06);} .menu .sep{background:rgba(0,0,0,.08);}}',
    '</style>',
    '<button class="btn" type="button" aria-label="Open torii menu" aria-haspopup="menu" aria-expanded="false">',
      '<svg class="logo" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" fill="#c8392c" aria-hidden="true">',
        '<path d="M2 8 C14 8, 22 16, 32 16 C42 16, 50 8, 62 8 L62 12 C50 12, 42 20, 32 20 C22 20, 14 12, 2 12 Z"/>',
        '<rect x="10" y="22" width="44" height="2"/>',
        '<rect x="30" y="22" width="4" height="8"/>',
        '<rect x="12" y="30" width="40" height="4"/>',
        '<path d="M14 58 L21 58 L20 24 L15 24 Z"/>',
        '<path d="M43 58 L50 58 L49 24 L44 24 Z"/>',
      '</svg>',
